...
alp2img = dict()
pro2img = dict()
charPaths = glob("ProChars/*png") + glob("AlpChars//*png")
for charPath in charPaths:
    print(charPath)
    ch = os.path.basename(charPath).split('.')[0]
    # img = cv2.imread(charPath)
    img = cv2.imdecode(np.fromfile(charPath, dtype=np.uint8), 1)
    img1 = cv2.resize(img, (22, 48))
    alp2img[ch] = img1
    img2 = cv2.resize(img, (23, 54))
    pro2img[ch] = img2
#     print(img.shape)
# exit(123)
...
class GenGreenPlate:
    ...
    def draw_string(self, text):
        ...
        # 调整字符间的间距
        # 第一个汉字
        self.img[8:62, offset + 8:offset + 8 + 23] = cv2.resize(pro2img[text[0]], (23, 54))
        self.pointG.append([(offset + 8 - 1, 8), (offset + 8 + 21 + 1, 8),
                            (offset + 8 - 1, 62), (offset + 8 + 21 + 1, 62)])  # p1(x,y),p2(x,y)...
        # 第二个字符
        self.img[8:56, offset + 8 + 22 + 6:offset + 8 + 22 + 6 + 22] = cv2.resize(alp2img[text[1]], (22, 48))
        self.pointG.append([(offset + 8 + 22 + 6 - 1, 8), (offset + 8 + 22 + 6 + 22 + 1, 8),
                            (offset + 8 + 22 + 6 - 1, 62), (offset + 8 + 22 + 6 + 22 + 1, 62)])
        # 后面五个字符
        for i in range(6):
            base = offset + 8 + 22 + 6 + 22 + 17 + i * 24
            self.img[8:56, base: base + 22] = cv2.resize(alp2img[text[i+2]], (22, 48))
            self.pointG.append([(base - 1, 8), (base + 22 + 1, 8),
                                (base - 1, 62), (base + 22 + 1, 62)])
        ...
        for unit, cpos in zip(box, range(len(box))):
            if unit == 1:
                ...
                if cpos == 0:
                    ...
                    # temp = r(34)
                    temp = r(10) + (r(24) if r(4) == 1 else 0) # 生成更多的数字牌
                    plateStr += chars[temp]
                    plateKey.append(get_dict_key(chars, chars[temp]))
        # print('str=' + plateStr)
        # print(plateKey)
    ...
    def getPlateImg(self, text):
        ...
        if len(text) == 8:
            ...
            # print(plate_img.shape)
            # 形态学变换
            plate_img, self.pointG = rot(plate_img, r(30) - 15, plate_img.shape, 20, self.pointG)
            plate_img, self.pointG = rotRandrom(plate_img, 5, (plate_img.shape[1], plate_img.shape[0]), self.pointG)
            ...
            # cv2.imshow('sst',plate_img)
            # cv2.waitKey(0)
            plate_img = AddGauss(plate_img, r(8))  # 高斯模糊
...
def test():
    ...
    G.genBatch(5000, "D:/plate_green", (420, 98))
